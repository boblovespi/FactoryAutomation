...
{
	private static boolean IterateOverMultiblock(World world,
			BlockPos controllerPos, String structureId, EnumFacing facing,
			IterateAction action)
	{
		...
		BlockPos lowerLeftFront = AddWithRotation(
				controllerPos, -offset[0], -offset[1], -offset[2], facing);
		...
		for (int x = 0; x < pattern.length; x++)
		{
			for (int y = 0; y < pattern[x].length; y++)
			{
				...
				{
					BlockPos loc = AddWithRotation(
							lowerLeftFront, x, y, z, facing);
					...
					System.out.println(
							"relative = ( " + x + " , " + y + " , " + z + " )");
					System.out.println(
							"part = " + part.GetBlock().getLocalizedName());
					System.out.println(
							"block in world = " + world.getBlockState(loc)
													   .getBlock()
													   .getLocalizedName());
					...
					} else
					{
						if (action == IterateAction.CHECK_VALID)
						{
							if (!pattern[x][y][z].MatchesBlockstate(
									world.getBlockState(loc)))
							{
								return false;
							}
						...
						} else if (action == IterateAction.CREATE)
						{
							...
							world.setBlockState(
									loc, FABlocks.multiblockPart.ToBlock()
																.getDefaultState());
							TileEntityMultiblockPart newPart = (TileEntityMultiblockPart) world
									.getTileEntity(loc);

							newPart.SetMultiblockInformation(structureId,
															 new BlockPos(x, y,
																		  z),
															 AddWithRotation(
																	 AddWithRotation(
	...
	public static boolean IsStructureComplete(World world, BlockPos pos,
			String id, EnumFacing facing)
	{
		return IterateOverMultiblock(
				world, pos, id, facing, IterateAction.CHECK_VALID);
	}

	private static BlockPos AddWithRotation(BlockPos pos, int x, int y, int z,
			EnumFacing dir)
	{
		switch (dir)
		{
