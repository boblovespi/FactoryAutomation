...
public class ConfiguredFeatures
{
	...
	//	public static ConfiguredFeature<?, ?> tinSmallVein = Register("ore_cassiterite_small", Feature.ORE.configured(new OreConfiguration(OreFeatures.STONE_ORE_REPLACEABLES, FABlocks.metalOres.GetBlock(MetalOres.TIN).ToBlock().defaultBlockState(), 4)));
	...
	{
		return null;// Feature.SIMPLE_BLOCK.configured(new SimpleBlockConfiguration(provider)).onlyWhenEmpty();
	}

	private static RandomPatchConfiguration SimplePatch(int tries, BlockStateProvider provider)
	{
		return null;// FeatureUtils.simpleRandomPatchConfiguration(tries, SimpleBlock(provider));
	}

	private static BlockState RockVariant(Variants variant)
	{
		return FABlocks.rock.defaultBlockState().setValue(Rock.VARIANTS, variant);
	}
}
